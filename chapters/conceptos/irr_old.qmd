...
\left\{
\begin{array}{l}
g(t) = \sum_{m=1}^{j} \frac{\text{ingresos}_m}{(1+i)^t} = \sum_{g=1}^{k} \frac{\text{gastos}_g}{(1+i)^t} \\
g(t) = \sum_{m=1}^{j} \frac{\text{movimiento}_m}{(1+i)^p} = 0 \\
p = \text{número de periodos respecto de } t
\end{array}
\right.
...
\left\{
\begin{array}{ll}
f(i) &= \sum_{m=1}^{^j}\frac{ingresos_m}{(1+i)^t} - \sum_{g=1}^{^k}\frac{gastos_g}{(1+i)^t} \\
f^`(i^`) &= \sum_{g=1}^{^k}\frac{gastos_g}{(1+i^`)^t} - \sum_{m=1}^{^j}\frac{ingresos_m}{(1+i^`)^t} \\
i &= -i^` 
...
f(i) = \lvert \sum \frac{\text{flujos}_m}{(1+i)^p} \rvert \leq 10^{-precision} 
...
$$
\forall i,j \: si \: i < j \rightarrow p_i \leq p_j \\
f(i) = \frac{flujo_1}{(1 + i)^0} + \frac{flujo_2}{(1 + i)^7} + \frac{flujo_1}{(1 + i)^7} + \frac{flujo_1}{(1 + i)^{14}} + \frac{flujo_1}{(1 + i)^{21}} + ...  \\
f(i) = \frac{flujo_1}{(1 + i)^{7 * 0}} + \frac{flujo_2}{(1 + i)^{7 * 1}} + \frac{flujo_1}{(1 + i)^{7 * 1}} + \frac{flujo_1}{(1 + i)^{7 * 2}} + \frac{flujo_1}{(1 + i)^{7 * 3}} + ...  
$$
